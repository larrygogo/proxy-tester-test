...
pub fn generate_accept_language() -> String {
    let mut rng = rand::thread_rng();
    // 定义一个真实的语言代码列表
    let language_codes = vec![
        "af", "ar", "az", "be", "bg", "bn", "bs", "ca", "cs", "cy", "da", "de", "el", "en", "es",
        "et", "fa", "fi", "fr", "ga", "gl", "gu", "he", "hi", "hr", "hu", "hy", "id", "is", "it",
        "ja", "ka", "kk", "km", "kn", "ko", "ku", "ky", "lt", "lv", "mk", "ml", "mn", "mr", "ms",
        "my", "ne", "nl", "no", "pa", "pl", "ps", "pt", "ro", "ru", "sd", "si", "sk", "sl", "so",
        "sq", "sr", "sv", "sw", "ta", "te", "tg", "th", "tr", "uk", "ur", "uz", "vi", "zh", "zu",
    ];
    let language_count = rng.gen_range(1..=5);

    let mut languages = Vec::new();
    for _ in 0..language_count {
        // 从语言代码列表中随机选择一个语言代码
        let language = language_codes.choose(&mut rng).unwrap();
...
fn default_headers() -> HeaderMap {
    let rua = fake_user_agent::get_rua();
    ...
    headers.insert("User-Agent", rua.parse().unwrap());
...
pub async fn to_create_session(product_id: &str, proxy: &str, timeout: u64) -> TestResult {
    ...
    let uid = {
        ...
        let _ = runtime.execute_script("<init>", UID_SCRIPT);
        let script = format!(
            "create_uid('{}', '{}', '{}')",
            "g",
            product_id,
    ...
    let create_url = format!(
        "https://api-ticketwaiting.interpark.com/v1/wyns/create/session/{}/{}?t={}&d=g",
...
mod tests {
    ...
    async fn test_query_itp_wait() {
        let proxy = "223.255.198.81:50218:oBMMY9zv:OiZfJBaJ";
        let split: Vec<&str> = proxy.split(':').collect();
        let proxy = format!("http://{}:{}@{}:{}", split[2], split[3], split[0], split[1]);
        let timeout = 30;
        let res = to_create_session("24004632", &proxy, timeout).await;
        println!("{:?}", res);
    }
